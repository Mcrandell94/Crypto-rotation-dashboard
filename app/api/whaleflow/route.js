...
export async function GET(request) {
  ...
  try {
    const json = await fetchCoinLobster('whale_flow', { coin }, apiKey, { revalidateSeconds: 300 });
    const rows = extractArray(json, ['hours', 'flow', 'data', 'results', 'items']);
    ...
    const hours = rows
      .map((h) => ({
        timestamp: normalizeTimeMs(pick(h, ['hour', 'time', 'timestamp', 'ts'])),
        buyUsd: Number(pick(h, ['buy_usd', 'buys_usd', 'buy'])) || 0,
        sellUsd: Number(pick(h, ['sell_usd', 'sells_usd', 'sell'])) || 0,
        cexBuyUsd: Number(pick(h, ['cex_buy_usd', 'cex_buy'])) || null,
        cexSellUsd: Number(pick(h, ['cex_sell_usd', 'cex_sell'])) || null,
        dexBuyUsd: Number(pick(h, ['dex_buy_usd', 'dex_buy'])) || null,
        dexSellUsd: Number(pick(h, ['dex_sell_usd', 'dex_sell'])) || null,
      }))
      .sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));

    return Response.json({ coin, hours, fetchedAt: new Date().toISOString() });
  } catch (err) {
    return Response.json(
